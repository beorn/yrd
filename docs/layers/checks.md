...
- `push` — at the receive door, before the PR exists (cheapest refusals, taught at push time; runs whether or not the push submits)
- `submit` — speculative, on the submitter's bay, when a PR is asked to merge (today's `bay.check`)
