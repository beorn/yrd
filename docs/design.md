...
| 2   | PR keeps GitHub semantics; Candidate owns merge groups | **Confirmed.** Strengthened: PR adopts GitHub's exact state shape — `state: "open" \| "closed"` + `merged: boolean` — replacing today's `pushed/submitted/needs-author/rejected/integrated/already-landed/withdrawn/canceled` enum (B3).                                                                                                           |
