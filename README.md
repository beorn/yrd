...
3. **Publish if useful.** `git push origin HEAD:refs/heads/fix-login` shares or preserves the branch. In a superproject, use `git super push` with the same explicit remote and refspec. Publishing is optional: a push alone leaves the branch outside the queue, a draft that the list and the watch show until it is submitted.
