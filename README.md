...
**Where yrd fits in your CI/CD.** You already have a pipeline: push a branch → CI runs your tests → someone merges when it's green → CD deploys main. **yrd takes over exactly one step: the merge.** Everything else stays yours. Your CI keeps running on branches for fast feedback — optional, even: yrd doesn't need it. The merge button becomes a queue: instead of a person clicking merge on a green PR, you run `yrd submit`, and yrd re-runs your checks — the same commands, listed in `.yrd.yml` — **on the merged result** (main + your change together), then merges. One at a time, in order. Nothing else pushes to main. Two branches can both be green and still break main when combined, because each was tested against an older main — that gap between "CI passed" and "main works" is the only thing yrd adds. Your CD doesn't change: it watches main as before, but now every commit there passed its checks *as merged*, so main is always deployable. In one line: **CI answers "does my branch pass?" — yrd answers "will main still pass once this merges?" — CD ships whatever's on main.**
