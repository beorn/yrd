...
- **Submit a branch, get a result.** You push a branch and submit it. The queue checks it in a fresh checkout, merges it into the target branch, and tells you what happened.
- **No server, no database, no web page.** Every fact the queue knows is a commit on a ref in the repository, under `refs/yrd/changes/`. Any clone that fetches those refs reads the whole state with plain `git log`.
...
- **One repository, many writers.** Without a queue, branches race to the target, and some are tested against a target that has already moved.
...
- **submitter**: whoever ran `yrd submit`, a person or an agent, named by the string given with `--notify`. Merged and failed go to them; stuck goes to the owner.
- **target**: the branch changes merge into, `main` unless the config says otherwise.
- **check**: one command the queue runs, on submit or on merge, named in `.yrd.yml` at the target.
- **result**: pass, fail or stuck, of a check or of a queue run. A fail is the submitter's: the check ran and its command exited non-zero. Stuck means the queue itself cannot go on (a crash, a missing script, a check past its time limit); it stops the queue, is nobody's fault, and goes to the owner.
- **fact**: one commit on the change's own ref recording one step and its result: opened, checked, merged, failed, stuck, sent. Facts are written once and never rewritten.
...
- **owner**: whoever repairs a stuck queue; a string the config names and the notify command delivers to.
...
Every command takes `--json`. `yrd submit` refuses the target branch itself: the target is not a change. `yrd check` checks out HEAD afresh, so uncommitted changes are not seen.

## The config, `.yrd.yml`

The queue's config is a file on the target branch. It is read from the target on every queue run, never from the change, so a branch cannot change the checks that judge it. The smallest config that does something:
...
2. **Check.** The next queue run takes every queued change, oldest first, into a fresh checkout of its head. Two built-in checks run first: the change shares history with the target, and every pin it moved points at a commit on that submodule's `main`. Then the `on: submit` checks run.
3. **Merge.** The first checked change in line is merged with the target in a fresh checkout. A third built-in check runs there: the `.yrd.yml` of the merged tree still parses, so no change can land a config the next run cannot read. Then the `on: merge` checks run. A pass moves the target to one merge commit (`--no-ff`, so the merge is visible in history) that names the change in its message (`Change: <branch>@<sha>`). One change merges per run; the rest are checked again at the new target on the next run. Merging several checked changes as one tested batch, and splitting a failed batch to find the culprit, is planned and not built.
...
**Others may push the target.** The queue is meant to be the only writer, but nothing stops a person from pushing `main` by hand (what GitHub calls a direct push), and the queue does not pretend otherwise. Every queue run walks the target's history since the queue started and reports each commit it did not make to the owner, naming the commit and every pin it moved. Then it goes on from the new base. The change at the front is checked again there. A push the queue was about to make onto the old base is refused by its own `--force-with-lease`. A rollback is a `git revert`, submitted through the queue like any other change.
...
A change's history is its own ref, `refs/yrd/changes/<branch>@<sha>`, one commit per fact: opened, then checked, then merged or failed or stuck, then sent. Each fact is a one-line sentence plus trailers, the `Key: value` lines at the end of a commit message: which change (`Branch:`, `Head:`, `Target:`), then per kind who submitted it and for which issue, which config judged it, which check failed and why, which merge commit landed it, and who was told and whether the message arrived. The last fact's trailers carry the whole state, so `yrd queue list` is one cheap read of the refs. A change's state is never stored; it is worked out from the facts and from history. A change whose head is already in the target's history is merged whatever its facts say. So a change merged by hand still shows merged, and gets its merged fact on the next run, marked `Merged-By: hand`.
