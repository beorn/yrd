...
5. **Notify.** Every ending runs the `notify` entries whose `on:` lists it, with the record as one JSON object on stdin.
   ...
   `record` names the ending (merged, failed, stuck or merged-direct). `endingId` identifies this particular ending: its event ID in the event queue, or its ending record OID in the legacy queue. `endedAt` is that ending's ISO time with a zone. The object also carries `change`, `submitter`, and `issue` when given.
