...
`ops-plan` records `plan.capturedAt`. `ops-apply` timestamps the cutover event and any replacement M2 with it. A delayed apply therefore dates those records at plan time, not apply time.
