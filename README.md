...
For a Record-to-Event migration, set the maintenance stop before planning and leave it set through apply or rollback. Apply replaces the legacy pause ref with a permanent maintenance record naming the new event tip in the same atomic push that creates the event queue. The later ops cutover keeps that legacy record so a stale writer cannot reopen intake. Resume the event queue only after the migration proof; after rollback, resume the restored legacy queue only after rollback proof.

**A stalled line.** A line can run rounds and still judge nothing. The service reads the line as stalled when changes wait and none has been merged, failed or recorded stuck for `health.stallAfter` (default 45 minutes). The clock starts at the later of the last judgement and the oldest waiting change's opening, so a submit into a line idle for hours does not page at once. A line with nothing waiting is idle, not stalled, and a paused line is never stalled. A stalled line makes the health probe report unhealthy with `queue-line-stalled`. Its cause says which of two shapes the service sees: a round still running its checks, naming the branch and how long it has run, or no round running while the last completed round judged nothing. It also names the oldest waiting change, the threshold and where the threshold came from. The page clears on the next judgement. The health document's `facts.flow` carries the waiting count, the oldest waiting change, the last judgement, and `unjudgedForMs`. While a round runs, it also carries `roundOpen`: when the round started and, refreshed on each heartbeat from the round's own journal, the change it works and the phase it is in, or `phaseUnread` with the reason the journal could not say. A slow-round page names that phase. The first round after the service starts is judged the same way: until a round has ended, the waiting count comes from the line that round journalled before any check. Past ten minutes with no judgement, `slow` is true there and the runner row in `yrd list` and `yrd watch` says so, before any page. Every round also journals its waiting count. A `stallAfter` that is not a duration, or is shorter than ten minutes (a round may legitimately run that long), is refused when the config loads.
...
5. **Notify.** Every ending runs the `notify` entries whose `on:` lists it, each with the record as one JSON object on stdin: `record` is its discriminant (merged, failed, stuck or merged-direct), followed by `change`, `submitter`, `issue` when one was given, then `reason`, `log`, and the branch's `failures` count for failed, or `merge` for merged. Notify commands choose recipients and compose delivery text. For a change's current ending, later rounds read receipts between that ending and the captured tip, and any receipt settles a name for that ending. Exit 0 told it. Exit 4 says the transport answered and refused, with the reason on the command's last stdout line: the sent record says `Not-Told: <name> refused=<reason>`, and that name is never sent this ending again. Any other exit, a timeout, or a command that could not run is no receipt, and the name gets exactly one more attempt on the next round; if that fails too, its record says `Not-Told: <name> undelivered=<why>`. Every later sent record of the ending repeats its `Not-Told:` values, and `yrd list` then names the queue's operator as next, for example `submitter not told: <reason>`. Removed names run nothing, and a newly declared name is still told. A first telling with no matching entries writes `none` once. Delivery is at-least-once: an unrecorded result can be retried after a crash.
