...
- Anyone can also push a submodule's `main` directly. The next candidate raises the gitlink to it, so those commits are tested with the whole product and merge with the next change; if they break the superproject, every candidate is stuck until a change carrying the superproject's fix merges. A direct merge of the superproject that moves a gitlink is reported with the gitlink's path.
