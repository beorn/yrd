...
- **One process, one machine.** By rule it is the only writer of the queue branch. A bypass is detected and reported, not prevented.
...
- **event**: one commit on the change's own ref recording one step and its result: opened, checked, merged, failed, stuck, sent. Events are written once and never rewritten.
...
- **bypass**: a commit on the queue branch that the queue did not make, what GitHub calls a direct push. Reported, never prevented.
...
notify:                                   # the same shape as checks: a name, when it runs, what runs
  - submitter:
      ...
      run: bun tools/yrd-notify.ts        # gets the event as one JSON object on stdin
  - supervisor:
      on: [stuck, merged-bypass]
      run: bun tools/yrd-notify.ts --to @cto
```

A key the queue does not read is refused, never ignored. Where the queue keeps its checkouts, logs and temp files is not in this file: `git config yrd.workdir <path>` on the clone that runs the queue, else `.git/yrd` in that clone. A check's environment is built, not inherited: `PATH`, `HOME`, `SHELL`, `LANG`, `USER`, `LOGNAME`, `LC_*`, `TMPDIR` under the working directory, plus `YRD_REPO`, `YRD_BASE_SHA` and `YRD_CANDIDATE_SHA`, plus the variables the check lists under `environmentPassthrough`.
...
<your repo>/
  ...
  .git/
    refs/yrd/changes/<branch>@<sha>         the events: one ref per change, one commit per event; on origin too
    refs/yrd/freeze                         frozen/unfrozen events; the latest commit is the state, on origin too
...
The submitter and the queue share only the remote. The submitter pushes a branch and the change's first event to origin; the queue fetches from origin into its own clone, works in fresh checkouts, and pushes merges back. It never reads a submitter's clone, so the two can be on different machines.

## How a change moves

1. **Submit.** One atomic push of the branch and of the change's first event. The branch is pushed with `--force-with-lease`, so a push that would overwrite another submitter's head is refused, loudly.
...
5. **Notify.** Every ending runs the `notify` entries whose `on:` lists it, each with the event as one JSON object on stdin: `event` (merged, failed, stuck or merged-bypass), `change`, `submitter`, `issue` when one was given, then `reason` and `log` for failed and stuck, or `merge` for merged. The queue composes no prose and knows no addresses; the commands do. A change ends once, so the same object sent again after a crash is the same message, not a second one.

**Bypasses.** The queue is meant to be the only writer of the queue branch, but nothing stops a person from pushing `main` directly, and the queue does not pretend otherwise. Every queue run walks the queue branch's history since the queue's first event and reports each commit it did not make as a `merged-bypass` event, naming the commit and every pin it moved. Then it goes on from the new base. The change at the front is checked again there. A push the queue was about to make onto the old base is refused by its own `--force-with-lease`. A rollback is a `git revert`, submitted through the queue like any other change. A submitted change whose head reaches the queue branch by a bypass still reads as merged; its merged event says `Merged-By: bypass`.
...
- A bypass that moves a pin is reported with the pin's path, because that is the one thing the built-in check above never sees.
...
A change's history is its own ref, `refs/yrd/changes/<branch>@<sha>`, one commit per event: opened, then checked, then merged or failed or stuck, then sent. Each event is a one-line sentence plus trailers, the `Key: value` lines at the end of a commit message: which change and queue (`Change:`, `Target:`), then per kind who submitted it and for which issue, which config judged it, which check failed and why, which merge commit landed it and which queue run made it, and which notify entries ran and whether they delivered. The last event's trailers carry the whole state, so `yrd queue list` is one cheap read of the refs. A change's state is never stored; it is worked out from the events and from history. A change whose head is already in the queue branch's history is merged whatever its events say, and gets its merged event on the next run.
...
| Package | What it is |
|---|---|
| `packages/yrd-queue-core` | the queue: submit, the queue read, the queue run, checks, events |
