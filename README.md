...
- **result**: pass, fail or stuck, of a check or of a queue run. Fail means the check ran and said no, and the change goes back to its submitter. Stuck means the queue could not judge the change at all, so the problem may be the queue's rather than the change's; see step 4 of [How a change moves](#how-a-change-moves).
...
- **gitlink**: a submodule pointer, the commit a superproject records for one of its submodules. A top-level gitlink behind its submodule's `main` is **raised**: the queue tests and merges the superproject with that `main` instead.
...
1. **Submit** it to wait its turn: `yrd submit fix-login --notify <who>`. If the queue branch has moved, rebase first (`git fetch origin main`, then `git rebase FETCH_HEAD`) or pass `--rebase`. Follow it with `yrd queue list` and `yrd queue show fix-login`; a failure's log is on the machine running the queue. After a fix, commit and submit again: the same head is a retry, a new head a new change.
2. **Merge** it now: `yrd merge fix-login` checks and merges that branch in the next run, ahead of the line and even while it is paused or stuck, reusing its change if already queued. Every check still runs; a fail goes back to its submitter. This is how a fix for a stuck queue lands. Planned, not built.
...
5. Returns the change: queued, not merged. The queue revalidates against the target at merge time.
...
4. **Fail or stuck.** A failing check fails the change. If the candidate raised a gitlink, the queue first reruns that phase's checks on the queue branch with the same raises and none of the change: a failing or unjudgeable base makes the change stuck instead. Crashes, missing scripts, timeouts, exit 2 and unreachable submodule remotes are stuck too. A stuck change keeps its place; the run still judges and merges the changes behind it, then exits 2. To clear it, fix the cause and run again, take the change out with `yrd queue withdraw`, or submit a new head that passes; the same content sticks again.
...
- **Working on a submodule by itself is supported.** Anyone can commit and push a submodule's `main` with plain Git, and the queue never needs to reach whoever pushed. Every candidate raises a top-level gitlink behind its submodule's `main`, so those commits are tested with the whole product and merge with the next change. With Git Super selected in `git config yrd.git`, every run, idle or paused, also reports a `main` moved outside the queue as an `observed` notice. If the commits break the superproject, every candidate is stuck on them until a change carrying the fix merges; `yrd merge` lands it. A direct merge of the superproject that moves a gitlink is reported with the gitlink's path.
...
| 2    | stuck: at least one change could not be judged; see step 4                                                                                                                          |
