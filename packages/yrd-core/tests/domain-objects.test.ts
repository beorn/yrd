...
describe("Yrd domain objects", () => {
  ...
  it("replays a captured v3 result frame identically cold and warm through a projection checkpoint", async () => {
    // Static captured bytes, not a frame generated by this checkout's writer:
    // PREP must prove the reader can replay the installed v3 journal without
    // letting the newly compiled v4 capability change its projection.
    const backing = createMemoryJournal<unknown>([
      {
        command: {
          id: "00000000-0000-7000-8000-000000000001",
        ...
        cause: {
          ...
          op: "counter.add",
          commandHash: "39be69cf64d894d89414805b07e24507c9980dea34be0f5a1bf61426167c77dd",
        },
        events: [
          {
    ...
    const cold = await createYrd(definition, { inject: { journal: cache.journal } })
    const coldSnapshot = structuredClone(cold.state())
    expect(cache.reads).toEqual([0])
    expect(coldSnapshot).toEqual({ counter: { value: 4 } })
    const saved = cache.stored()
    expect(saved).toMatchObject({
      cursor: 1,
      value: {
        receipts: [expect.objectContaining({ compatibility: { version: 3 } })],
      },
    })
    if (saved === undefined) throw new Error("expected the cold replay to save a projection checkpoint")
    await cold.close()

    cache.reads.length = 0

    await using warm = await createYrd(definition, { inject: { journal: cache.journal } })

    expect(cache.reads).toEqual([1])
    expect(warm.state()).toEqual(coldSnapshot)
    expect(cache.stored()).toEqual(saved)
  })
