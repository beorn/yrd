...
export type GatherOptions = Readonly<{
  repo: string
  base: string
  /** Ref-local observation/update time selected by the enumerating boundary.
   * Commit time is payload metadata and must never be substituted here. */
  pushedAtMs: number
...
export async function gatherPushedRefFact(git: RefGit, ref: string, options: GatherOptions): Promise<PushedRefFact> {
  const { repo, base, carriedBranches, gitlinkPaths, pushedAtMs } = options
