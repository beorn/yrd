 ...
 * (stale head) while the component work its pin names already landed on the
 * component's own main (spent pin).
 *
 ...
 *
 * The pair read here is the one the promotion loop uses: authored pin against
 * component main, not against the pin the root base happens to carry. Those are
 * different questions — the root's gitlink lags component main by exactly the
 * promotion this carrier would have performed, so it answers "has root absorbed
 * this pin" and always says no for a carrier in this state. */
async function spentGitlinkCarrier(
  ...
  for (const path of authored) {
    ...
    if (carrierPin === undefined || (await readGitlink(git, candidate, authoritativeBase, path)) === undefined) {
      return undefined
    }
    ...
    // tell an author their work landed.
    const component = await componentCheckout(git, repo, path)
    if (component === undefined) return undefined
    const componentMain = await fetchComponentMain(git, component, "origin")
    if (componentMain.status === "failed") return undefined
    if (!(await isAncestor(git, component, carrierPin, componentMain.sha))) return undefined
    landings.push(`pin '${carrierPin}' for '${path}' is contained in component main '${componentMain.sha}'`)
  }
  return { landings }
}
