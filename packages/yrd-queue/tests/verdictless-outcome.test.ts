 ...
 * was consumed.
 *
 * TWO producers, one field. This file covers the COMMAND-level outcomes, where
 * a process ran and a watchdog or a signal ended it. The JOB-level ones live in
 * queue.test.ts beside their own cancel and lease coverage, because that is
 * where the harness that can cancel a live Job already is: a cancelled run
 * (`run-canceled`), a lost Job (`job-lost`) and a skipped step (`job-skipped`)
 * are all synthesized by `jobFailure`, and none of them judged anything either.
 * The live specimen for that half, 2026-09-01 15:11 PDT: a second one-shot pass
 * cancelled the first pass's run, and PR2916's `affected-tests` reported
 * "failed without a verdict (run-canceled)" — the message said it and the data
 * did not.
 * @level l2
