...
describe("an environment refusal inside a check never retires the submission", () => {
  it.each([
    ...
    expect(actions).toContain("submit-fact-kept-standing")
    const kept = events.find((event) => event.kind === "log" && event.props?.action === "submit-fact-kept-standing")
  ...
  it("a judged red still retires the fact (control): only an author-owned refusal can", async () => {
    ...
    expect(actionsLogged(events)).not.toContain("submit-fact-kept-standing")
  })
})
