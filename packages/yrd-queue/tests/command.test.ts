...
describe("Queue command adapters", () => {
  ...
  it("redirects an invalid manual composition away from the authored-root recut flow", async () => {
    const { repo, candidate } = await repository("candidate")
    const baseSha = await git(repo, ["rev-parse", "main"])
    await using process = createProcess()
    await using app = await checkedQueue(process, repo, ["true"])
    ...
    expect(run).toMatchObject({
      ...
      error: {
        ...
        message: expect.stringMatching(/root code must be submitted separately from component pin intents/iu),
      },
    })
    expect(run.error?.message).not.toContain("yrd pr recut")
  })
  ...
  it("renews one runner lease only on child progress and recovers a stalled child without merge", async () => {
    ...
    const controlledQueue = async () => {
      ...
      const merge = withMerge(
        (input: StepExecution<CheckedCommand>) => {
          mergeRuns.push("merge")
          const changeId = input.prs[0]?.changeId
          if (changeId === undefined) throw new Error("missing fixture Change-Id")
          return {
            status: "completed",
  ...
  it("checks and lands the exact Change-Id-stamped Candidate", async () => {
    ...
    if (run.integration === undefined) {
      throw new Error(`merge did not produce IntegrationProof: ${JSON.stringify(run.error)}`)
    }
    ...
    const note = JSON.parse(await git(repo, ["notes", "--ref=yrd/receipts", "show", checked.candidateSha])) as unknown
    expect(note).toEqual({
    ...
    expect(integration).toMatchObject({
      receipt: {
        ...
        note: noteObject,
        checksum: (note as { checksum: string }).checksum,
      },
    })
  ...
  it("recovers a landing-before-receipt crash without reclassifying the Queue landing as already-contained", async () => {
    ...
    expect(first).toMatchObject({
      status: "completed",
      conclusion: "failure",
      error: { code: "repository-incomplete" },
    })
  ...
  it.each(["native", "configured"] as const)(
    ...
    async (mode) => {
      ...
      expect(run).toMatchObject({ status: "completed", conclusion: "success" })
      const integration = IntegrationProofSchema.parse(run.integration)
      expect(integration.receipt).toMatchObject({
        ref: "refs/notes/yrd/receipts",
        target: integration.commit,
      })
      expect(
        JSON.parse(await git(fixture.rootRemote, ["notes", "--ref=yrd/receipts", "show", integration.commit])),
      ).toMatchObject({
        schema: "yrd/landing-receipt/v1",
        checksum: integration.receipt?.checksum,
      })
      expect((run.integration as unknown as { componentMains?: unknown }).componentMains).toEqual([
        {
  ...
  /**
   * The misclassification the head-shaped check produces, and its boundary. The
   * two tests below are a matched pair and must be read together: they differ in
   * one fact and take opposite cures, and reading the wrong one of the two shas
   * inverts the verdict on both.
   *
   * SPENT (this one): the pin the carrier authors is already contained in the pin
   * the AUTHORITATIVE BASE carries — another carrier performed the promotion
   * first. Rebuilding delivers nothing, so "linear rebuild required" is advice
   * that cannot succeed: the rebuild is empty and is refused again on the same
   * ground. That is the loop PR562 rode to revision 43.
   *
   * LIVE (the sibling below): component main holds the work, but root main's
   * gitlink still points before it — so this carrier is the one that would
   * perform the promotion, and telling its author to close would DROP the
   * advance. Measured directly: with root's gitlink behind, the rebuild still
   * delivers the gitlink; with root's gitlink containing the pin, it delivers
   * nothing.
   */
  it("tells a stale-headed gitlink-only carrier to close when the base already carries its pin", async () => {
    ...
    expect(message).toContain(fixture.successorSha)
  ...
  it("keeps the rebuild remedy when component main has the work but the base's pin does not", async () => {
    ...
    expect(run).toMatchObject({
      ...
      error: {
        code: "carrier-drops-landed",
        message: expect.stringMatching(/linear rebuild/iu),
      },
    })
  }, 20_000)
