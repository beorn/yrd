 ...
 * landings themselves: each successful merge advances `head`, and a merge whose
...
async function drainDrill(
  ...
  const merge = withMerge(
    (input: StepExecution<ReviewedShape>): JobResult<IntegrationProof> => {
      ...
      const commit = sha(`landing:${landings}:${members.join(",")}`)
      head = commit
      landings += 1
  ...
  return { builds, landings, checks, staleRefusals, cycles, integrated: integratedCount(), submitted }
}
 ...
 *
 * Exact and reproducible. `landings` equals the number of batch-groups
...
const MEASURED: readonly Point[] = [
  { candidates: 8, batch: false, builds: 36, landings: 8, checks: 36, staleRefusals: 28, cycles: 8 },
  { candidates: 10, batch: false, builds: 55, landings: 10, checks: 55, staleRefusals: 45, cycles: 10 },
  { candidates: 12, batch: false, builds: 78, landings: 12, checks: 78, staleRefusals: 66, cycles: 12 },
  { candidates: 10, batch: 2, builds: 15, landings: 5, checks: 15, staleRefusals: 10, cycles: 5 },
  { candidates: 10, batch: 4, builds: 6, landings: 3, checks: 6, staleRefusals: 3, cycles: 3 },
  { candidates: 10, batch: 10, builds: 1, landings: 1, checks: 1, staleRefusals: 0, cycles: 1 },
]
...
describe("queue drain throughput — rebuilds per landing", () => {
  ...
  it(
    ...
    async () => {
      ...
      for (const point of MEASURED) {
        ...
        expect(drill.builds).toBeGreaterThanOrEqual(drill.landings)
        const groups = Math.ceil(point.candidates / (point.batch === false ? 1 : point.batch))
        expect(drill.landings, "one landing per batch-group").toBe(groups)
  ...
  it.fails(
    ...
    async () => {
      ...
      const buildsPerLanding = drill.builds / drill.landings
      expect(
        buildsPerLanding,
        `${drill.builds} builds and ${drill.checks} checks to land ${drill.landings} disjoint candidates ` +
