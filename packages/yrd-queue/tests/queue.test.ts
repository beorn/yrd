...
describe("Queue", () => {
  it("refuses to run a request-bound PR when the host settlement capability is missing", async () => {
    const journal = createMemoryJournal()
    const id = ids()
    const terminal = createPRTerminalJobDef({
      revision: "test-terminal-v1",
      settle(input) {
        return { status: "passed", output: { requestId: input.requestId, disposition: "settled" } }
      },
    })
    const configured = await createQueueApp({ terminal }, journal, undefined, id)
    await submitBranch(configured, "issue/config-required", "main", "request-61")
    await configured.close()

    await using unconfigured = await createQueueApp({}, journal, undefined, id)
    await expect(unconfigured.queue.run({ prs: ["PR1"] }, runtime)).rejects.toThrow(
      "no PR terminal settlement is configured",
    )
    expect(unconfigured.bays.pr("PR1")).toMatchObject({ status: "submitted", requestId: "request-61" })
    expect(unconfigured.state().queues.records).toEqual({})
  })
