...
describe("persistent Core projection checkpoint", () => {
  it("restores state and retry registries, then folds only the post-checkpoint tail", async () => {
    ...
    const seeded = JSON.parse(await readFile(join(dir, "snapshot-v4.json"), "utf8")) as {
    ...
    const tail = await createYrd(definition, { inject: { journal: createJournal({ dir }), id } })
  ...
  it("rejects a checkpoint from different reducer semantics and rewrites it from journal authority", async () => {
    ...
    const before = JSON.parse(await readFile(join(dir, "snapshot-v4.json"), "utf8")) as {
    ...
    const after = JSON.parse(await readFile(join(dir, "snapshot-v4.json"), "utf8")) as {
      checkpoint: { identity: string }
    }
