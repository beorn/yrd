...
describe("runYrd", () => {
  ...
  it("records a plain submit for a later Queue drain and uses --follow as the synchronous opt-in", async () => {
    const checkRuns: string[] = []
  ...
  it("builds one filtered one-revision timeline and deduplicated STATS projection", async () => {
    ...
    const prs = [
      ...
      timelineFixturePr(pr.id, pr.status as Exclude<PRDeliveryState, "needs-author">, pr.submittedAt, pr.name, {
