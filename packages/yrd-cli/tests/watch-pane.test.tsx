...
import { defaultCheckIndex, type CheckPanel } from "../src/watch-detail.tsx"
...
function snapshot(over: Partial<WatchSnapshot> = {}): WatchSnapshot {
  ...
  return {
    at: NOW,
    detail: new Map([[failed.head, { checks: CHECKS, row: failed }]]),
...
describe("the top line", () => {
  ...
  it("puts the pause above the name, because a queue that is not running is the loudest thing about it", async () => {
    const text = await paint(<WatchPane snapshot={snapshot({ pause: "paused by @chief: the host is down" })} live={false} />)
  ...
  it("says WHERE the run journal was looked for when there was none, so no journal never reads as nothing running", async () => {
    const text = await paint(
      <WatchPane snapshot={snapshot({ journalAbsent: "no run journal was read: /w/logs — there is no such directory" })} live={false} />,
    )
