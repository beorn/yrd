...
describe("synchronized activity pulse (items 12-13)", () => {
  it("pulses only the RUNNER marker while the command stays static (round-3 d)", async () => {
    ...
    try {
      ...
      expect(app.text).toContain("$ habitant runner [84042]")
      const markerBefore = cellOf(app, "$", "habitant runner").fg
      const commandBefore = cellOf(app, "habitant", "habitant runner").fg

      await vi.advanceTimersByTimeAsync(900)
      app.rerender(tree())

      expect(cellOf(app, "$", "habitant runner").fg, "marker advances to the next pulse phase").not.toEqual(
        markerBefore,
      )
      expect(cellOf(app, "habitant", "habitant runner").fg, "command remains static across marker phases").toEqual(
        commandBefore,
      )
    } finally {
      app.unmount()
