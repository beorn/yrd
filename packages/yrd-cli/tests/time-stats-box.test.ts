...
describe("QueueStatsPanel", () => {
  ...
  it("adapts only the newest-first local hour columns while retaining all fixed periods", () => {
    expect([48, 49, 54, 80, 145, 160].map(queueStatsHourCount)).toEqual([0, 0, 1, 6, 19, 22])
    ...
    const hourRender = createRenderer({ cols: 54, rows: 30 })
    const hour = hourRender(boxesElement({ facts: FACTS, now: NOW, earliestFactMs: HORIZON, width: 54 }))
    expect(rowContaining(hour, "TOTAL")).not.toContain("…")
    expect(rowContaining(hour, "TOTAL")).toContain("~2m")
  })
