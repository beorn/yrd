...
import { QUEUE_HEALTH_DOCUMENT, QUEUE_HEALTH_SCHEMA, roundHealthDocument } from "@yrd/queue-core"
...
describe("the declared health probe", () => {
  it("prints what the last round wrote, and exits on its state", () => {
    const dir = workdir()
    const written = roundHealthDocument(SERVICE, {}, undefined, 120_000)
    writeFileSync(join(dir, QUEUE_HEALTH_DOCUMENT), `${JSON.stringify(written, undefined, 2)}\n`)
    const run = capture()
    expect(queueHealthCommand(dir, SERVICE, run.io)).toBe(0)
    expect(JSON.parse(run.stdout())).toEqual(written)
  })

  it("reports a stuck round as unhealthy-and-running, exit 2", () => {
    const dir = workdir()
    const streak = { reason: "the code host answered 504 during setup", consecutive: 2 }
    const written = roundHealthDocument(SERVICE, { stuck: streak.reason }, streak, 240_000)
    ...
    expect(queueHealthCommand(dir, SERVICE, run.io)).toBe(2)
  ...
  it("says ABSENT when no round has written one, exit 1", () => {
    const run = capture()
    const dir = workdir()
    expect(queueHealthCommand(dir, SERVICE, run.io)).toBe(1)
  ...
  it("says UNKNOWN and quotes the text when the document is broken, exit 3", () => {
    ...
    const run = capture()
    expect(queueHealthCommand(dir, SERVICE, run.io)).toBe(3)
