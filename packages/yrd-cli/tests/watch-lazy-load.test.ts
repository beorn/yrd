...
describe("watch UI lazy-load contract", () => {
  // watchQueue (the one use site) lives in the queue command module; run.ts keeps
  // a type-only import for the snapshot types. Both sit on every CLI boot path.
  const watchPaneImporters: readonly [file: string, specifier: string][] = [
    ["src/run.ts", "\\.\\/watch-pane\\.tsx"],
    ["src/commands/queue.ts", "\\.\\.\\/watch-pane\\.tsx"],
  ]
  ...
  it("imports only types statically from the watch-pane module", () => {
    ...
    for (const [file, specifier] of watchPaneImporters) {
      const source = src(file)
      const staticImports = [
        ...source.matchAll(new RegExp(`^import\\s+(type\\s+)?\\{[^}]*\\}\\s+from\\s+"${specifier}"`, "gmu")),
      ]
      expect(staticImports, `${file} static watch-pane imports`).toHaveLength(1)
      expect(staticImports[0]?.[1], `${file} watch-pane static import must be type-only`).toBe("type ")
    }
  })

  it("reaches the live watch UI through a dynamic import at its use site", () => {
    expect(src("src/commands/queue.ts")).toMatch(/await import\("\.\.\/watch-pane\.tsx"\)/u)
  })
