...
export type QueueWatchSnapshot = Readonly<{
  ...
  /** `needs-person` findings — merge requests whose admission refusal
   * settled `needs-person` and stopped being retried, so no other row in
   * this pane marks them (@i/10-merge-queue/22918-needs-person-unowned). A
   * visible row, never only a `queue audit` record nobody ran; no age
   * threshold, unlike `staleDrafts`, since a settlement already earned its
   * urgency. Absent/empty both mean "nothing needs a person". */
 ...
 * snapshot carries — same shape as {@link staleDraftFooterNotice}, but a
 * settlement already earned its urgency (no page-after grace, unlike a
