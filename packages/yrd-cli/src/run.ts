...
function indexRebuildLines(report: IndexRebuildReport): readonly string[] {
  ...
  const lines = [
    ...
    // `changes` counts every change in every merged attempt; one PR can appear in
    // several, and only its latest merged attempt describes the landing. Naming
    // that collapse is what turns an unexplained shortfall into arithmetic the
    // reader can check: rebuilt + skipped = the distinct landings considered.
...
async function configDoctor(
  ...
  // findings and an unrebuilt gap are. `submoduleTrackingWarnings` fires on ANY
  // unbranched submodule, so every run inside a superproject carried at least one
  // warning and `clean` was false before doctor had looked at anything — which
  // made the exit code a constant 1 there, and made the unrebuilt gap it is
  // supposed to expose unobservable. Warnings are still printed; they just no
