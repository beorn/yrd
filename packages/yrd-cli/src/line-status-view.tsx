...
function appendUnknownArray(target: unknown[], value: unknown): void {
  if (!Array.isArray(value)) return
  for (const item of value as unknown[]) target.push(item)
}
...
function stepArtifacts(step: LineStep | undefined): readonly unknown[] {
  ...
  if ("artifacts" in step.job) appendUnknownArray(artifacts, step.job.artifacts)
  if ((step.job.status === "passed" || step.job.status === "failed") && isObjectValue(step.job.output)) {
    appendUnknownArray(artifacts, step.job.output.artifacts)
  }
  const checkpoint = jobCheckpoint(step.job)
  if (isObjectValue(checkpoint)) appendUnknownArray(artifacts, checkpoint.artifacts)
