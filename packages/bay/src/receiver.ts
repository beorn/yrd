...
export async function createGitPushReceiver(options: {
  ...
  const mainFormat = parseObjectFormat((await mainGit(mainRepo, ["rev-parse", "--show-object-format"])).stdout)
  const lock = await acquireWriterLock(join(stateDir, "receiver-init"), { timeoutMs: 30_000, pollIntervalMs: 10 })
  try {
    ...
    const receiverFormat = await bareFormat(receiverPath)
    check(
    ...
    if ((await mainGit(mainRepo, ["for-each-ref", "--format=%(refname)", "refs/heads"])).stdout !== "") {
...
export async function loadGitPushReceiver(path: string): Promise<GitPushReceiver> {
  ...
  const format = await bareFormat(receiverPath)
  const version = await requiredConfig(receiverPath, "yrd.receiverVersion")
  check(version === String(RECEIVER_VERSION), `unsupported receiver version '${version}' at '${receiverPath}'`)
  const receiver: GitPushReceiver = Object.freeze({
    version: RECEIVER_VERSION,
    receiverPath,
    mainRepo: resolve(await requiredConfig(receiverPath, "yrd.mainRepo")),
    stateDir: resolve(await requiredConfig(receiverPath, "yrd.stateDir")),
    inboxDir: resolve(await requiredConfig(receiverPath, "yrd.inboxDir")),
    ...format,
  })
...
export async function finalizeReceiverUpdates(
  ...
  if (options.intake) await drainReceiverInbox(receiver, { ...options, intake: options.intake })
  return receipts
}
...
export async function runReceiverHookFromEnvironment(
  ...
  const receiver = await loadGitPushReceiver(resolve(process.cwd(), env.GIT_DIR))
  const input = options.input ?? (await Bun.stdin.text())
  if (mode === "pre-receive") return prepareReceiverUpdates(receiver, input, { ...options, env })
  if (mode === "post-receive") return finalizeReceiverUpdates(receiver, input, { ...options, env })
...
async function exec(argv: readonly string[], cwd: string, options: ExecOptions = {}): Promise<Result> {
  const child = Bun.spawn([...argv], { cwd, env: options.env ?? gitEnv(), stdout: "pipe", stderr: "pipe" })
  const [stdout, stderr, exitCode] = await Promise.all([
    new Response(child.stdout).text(),
    new Response(child.stderr).text(),
    child.exited,
  ])
  const result = { code: exitCode, stdout: stdout.trim(), stderr: stderr.trim() }
  check(
    options.allowFailure || exitCode === 0,
    `${argv.join(" ")} failed: ${result.stderr || result.stdout || `exit ${exitCode}`}`,
  )
  return result
}
...
async function mainGit(repo: string, args: readonly string[], options: ExecOptions = {}): Promise<Result> {
  return exec(["git", "-C", repo, ...args], repo, { ...options, env: gitEnv(options.env) })
}
...
async function receiverGit(
  ...
  const env = includeMainObjects ? await objectEnv(receiver.mainRepo, options.env) : gitEnv(options.env, true)
  return exec(["git", `--git-dir=${receiver.receiverPath}`, ...args], receiver.receiverPath, { ...rest, env })
}

async function objectEnv(mainRepo: string, source: Environment = process.env): Promise<Environment> {
  const objects = resolve(
    mainRepo,
    (await mainGit(mainRepo, ["rev-parse", "--git-path", "objects"], { env: source })).stdout,
  )
...
async function bareFormat(path: string): Promise<{ objectFormat: "sha1" | "sha256"; shaLength: 40 | 64 }> {
  const bare = await exec(["git", `--git-dir=${path}`, "rev-parse", "--is-bare-repository"], path, {
    allowFailure: true,
  })
  check(bare.code === 0 && bare.stdout === "true", `'${path}' exists but is not a bare Git repository`)
  return parseObjectFormat((await exec(["git", `--git-dir=${path}`, "rev-parse", "--show-object-format"], path)).stdout)
}
...
async function config(path: string, key: string): Promise<string | undefined> {
  const result = await exec(["git", `--git-dir=${path}`, "config", "--local", "--get", key], path, {
    allowFailure: true,
  })
  check(
...
async function requiredConfig(path: string, key: string): Promise<string> {
  const value = await config(path, key)
  check(value, `'${path}' is missing required config '${key}'`)
  return value
}

async function validateBinding(receiver: GitPushReceiver): Promise<void> {
  const version = await config(receiver.receiverPath, "yrd.receiverVersion")
  const state = await config(receiver.receiverPath, "yrd.stateDir")
  const main = await config(receiver.receiverPath, "yrd.mainRepo")
  const inbox = await config(receiver.receiverPath, "yrd.inboxDir")
...
async function validatePin(
  ...
  const current = await mainGit(receiver.mainRepo, ["rev-parse", "--verify", `refs/heads/${target.base}^{commit}`], {
    env,
    allowFailure: true,
  })
  check(current.code === 0, `base branch '${target.base}' does not resolve in the main repository`)
  const pinned = await mainGit(receiver.mainRepo, ["merge-base", "--is-ancestor", target.baseSha, current.stdout], {
    env,
    allowFailure: true,
  })
  check(
