...
export function withContests(options: WithContestsOptions): ContestPlugin {
  ...
  const install = <State extends object, Commands extends CommandTree, Features extends ContestFeatures>(
    ...
    definition.extend({
      ...
      commands,
      events: {
        "contest/opened": journalEvent(1, OpenedSchema),
        "contest/selected": journalEvent(1, ContestSelectArgsSchema),
        "contest/promotion/requested": journalEvent(1, PromotionRequestSchema),
        "contest/promoted": journalEvent(1, PromotedSchema),
      },
