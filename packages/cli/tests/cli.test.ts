...
type CheckOutput = { checked: true }
type CheckedShape = AddStepResult<SubmissionShape, "check", CheckOutput>
...
function workspace(
  options: { dirty?: boolean; refreshedHead?: string; probe?: OverlapProbe } = {},
): BayWorkspaceAdapter {
  return {
...
function contestAdapters(probe?: OverlapProbe, baseResolutions?: string[]) {
  ...
  const evaluator: ContestEvaluatorAdapter = {
    id: "held-out",
    authority: "held-out",
    async evaluate() {
...
function createApp(
  ...
  return pipe(
    ...
    withMerge(
      (_input: StepExecution<CheckedShape>): EffectOutcome<{ commit: string; baseSha: string }> => ({
        status: "passed",
        output: { commit: MERGED_SHA, baseSha: MERGED_SHA },
      }),
    ),
    withContests({ runners: [contest.runner], evaluators: [contest.evaluator], git: contest.git }),
  )
}
...
async function openAndSubmit(app: ReturnType<typeof createApp>): Promise<void> {
  const open = outputIO()
...
describe("runYrd", () => {
  ...
  it("opens, refreshes, and closes bays through installed command refs while driving effects", async () => {
    ...
    expect(state.effects.runs[state.bays.bays.B1!.effectId!]?.status).toBe("passed")
  ...
  it("submits inferred bays and runs selected line steps instead of merely enqueueing effects", async () => {
    const app = createApp()
  ...
  it("refreshes an active bay before submit and refuses uncommitted work", async () => {
    ...
    const clean = createApp({ refreshedHead })
    const open = outputIO()
    ...
    const dirty = createApp({ dirtyBay: true })
    ...
    expect(Object.keys((await dirty.state()).bays.submissions)).toEqual([])
  })
  ...
  it("runs a real task contest to durable evidence, then selects and promotes the exact winner", async () => {
    ...
    expect(JSON.parse(promote.stdout())).toMatchObject({
      ...
      contest: { status: "promoted", promotion: { attempt: "A1", status: "passed" } },
    })
  })
  ...
  it("uses the documented exit taxonomy and keeps diagnostics off stdout", async () => {
    ...
    broken.line.status = async () => {
      throw new Error("corrupt event log at row 4")
    }
    const infrastructure = outputIO()
    expect(await runYrd(broken, yrd("line", "status"), infrastructure.io)).toBe(3)
  ...
  it("projects installed line administration and cancels an idle watch deterministically", async () => {
    const app = createApp()
    Object.assign(app.line, {
      audit: async () => ({ findings: [{ code: "operator-finding", message: "inspect runner" }] }),
    ...
    expect(await runYrd(app, yrd("line", "provision", "release/2.0", "--json"), provision.io)).toBe(0)
    ...
    expect(await runYrd(app, yrd("line", "audit", "--json"), audit.io)).toBe(1)
    ...
    expect(await runYrd(app, yrd("line", "watch", "--interval", "1"), watch.io)).toBe(0)
