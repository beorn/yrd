 ...
 * events at the moment you ask:
 *
 * - `queued` — an opened event and no checked event after it;
 * - `checked` — a checked event and no ended event after it;
 * - `stuck` — the last event ended with stuck; the change stays open;
 * - `merged` — the head is an ancestor of the target;
 * - `failed` — the last event ended with failed, or the branch no longer carries
...
export type ChangeEvents = Readonly<{
  /** The change's own branch. Never the target: that one is `QueueRunOptions.target`. */
  branch: string
  /**
   * The change's events, oldest first, or only its tip: every reading here uses
   ...
   * its first event (E2). Every reader used to ask anyway and invent an answer
...
/**
 * The change's tip: the event whose trailers are the whole derived state.
 *
 * The `??` is what the language costs to say what the type already knows —
 * `at(-1)` is `Event | undefined` for any tuple, while the tuple's first
 * element is a `Event` outright, and on a change with one event they are the
 * same event.
 */
export function tipOf(change: ChangeEvents): Event {
  return change.events.at(-1) ?? change.events[0]
}

/** Read one change's state. Pure: every input is an event or a git reading. */
export function readChange(change: ChangeEvents): ChangeReading {
  // Ancestry first, and before anything the events say.
  ...
  switch (last.kind) {
    ...
    case "opened":
      return { state: "queued" }
    case "sent": {
      // A sent event repeats the ended state it followed (`State:`, ruling A2)
      // and carries that event's result, so the tip alone answers.
      ...
      throw new Error(`sent event ${last.sha.slice(0, 12)} names no ended state (State: ${state ?? "absent"})`)
    }
  }
}

/**
 * Position in line: the order of opened events by their commit time
 ...
 * the next queue run takes it again (§ The words). Opened events are the one
 ...
 */
export function inLine(changes: readonly ChangeEvents[]): readonly ChangeEvents[] {
  return [...changes]
    .filter((change) => {
...
/** When the change was first opened, carried on every event as `Opened:`. A change with no events has no place in line, and no existence (E2). */
export function openedAt(change: ChangeEvents): number {
  const last = tipOf(change)
  const opened = last.trailers.find(([name]) => name === "Opened")?.[1]
  const time = opened === undefined ? Number.NaN : Date.parse(opened)
  if (Number.isNaN(time)) throw new Error(`event ${last.sha.slice(0, 12)} carries no readable Opened: (${opened ?? "absent"})`)
  return time
}

/**
 * A failed event's `Reason` (a check's name, conflict, config-invalid,
 * unrelated-history, gitlink-off-main, replaced, deleted) or a stuck event's
 * (a check's name, setup, crash): one key on both, because stuck is always the
 * queue's and needs no second word for it.
 */
function reasonOf(event: Event): string | undefined {
  return event.trailers.find(([name]) => name === "Reason")?.[1]
}
