...
describe("settling gitlinks", () => {
  ...
  it("classifies a nested pin whose checkout holds only its .git, as production does", async () => {
    ...
    const head = await submitGitlink(w, "task/nested-bare", ahead)
    ...
    expect(
      await w.git([
        "show",
        "-s",
        "--format=%(trailers:key=Settled,valueonly)",
        await remoteTip(w.git, "refs/heads/main"),
      ]),
      "and it is classified, not refused as a missing store",
    ).not.toContain("gitlink-store-absent")
