...
Batching is automatic when `bay.queue.batch-size` is greater than one: `git bay integrate` composes compatible queued PRs into a batch candidate, lands that candidate through the existing merge command, and records the batch in the journal-backed status output.
...
bay: batch PR3 built — members: PR1, PR2
bay: PR3 queued → merging
bay: PR3 merging → rejected — exit 7: bad batch
