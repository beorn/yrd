/**
 * @failure A change's events are the queue's whole store. The plan says a
 *          submit opens `refs/yrd/changes/<branch>@<head>`, that each event is
 ...
 *          the tip event's trailers are the entire answer for that change.
 *          Nothing today pins any of it, so a core could record a change in a
 *          private store, edit an event, or answer `queue list` from a walk, and
 ...
 * and the format it cites, @i/10-yrd/event-commit-format-2026-09-02. Never from
 ...
 *
 * WHERE THE EVENTS ARE READ. The plan says a branch is its ref "at the `yrd`
...
/** The six values `Event:` may carry, and no others. */
const EVENT_KINDS = new Set(["opened", "checked", "merged", "failed", "stuck", "sent"])
...
async function readEvents(boundary: BoundaryRepository, change: Readonly<{ branch: string; headSha: string }>) {
...
describe("a change and its events", { timeout: 120_000 }, () => {
  describe("the change ref", () => {
    ...
    it("a submit opens refs/yrd/changes/<branch>@<head> in the shared repository", async () => {
      ...
      const read = await readEvents(boundary, change)
    ...
    // today: red — no change ref, so no event commit to shape.
    it("the opened event is one commit: a prose first line, then trailers, one meaning each", async () => {
      ...
      // One meaning each: an event carries exactly one Event: trailer.
      expect(opened.trailers.get("Event"), read.report).toEqual(["opened"])
    ...
    it("the opened event names the submitter, the time, the target and the issue", async () => {
      // A branch under the convention `<issue>-<slug>`, so there is a work
      // item to name.
      const { boundary, change } = await submitted({ exit: 0 }, "24101-events")
      ...
      // the opened events by commit time — so it has to be on the commit.
    ...
    it("the opened event's parents are genesis first and the head second, so the head cannot be collected", async () => {
    ...
    it("git log --first-parent on the change ref reads exactly the events, never the project's history", async () => {
      ...
      expect(read.exists, read.report).toBe(true)
      // One opened event and the genesis it hangs from. Nothing else.
      expect(read.firstParentLine, read.report).toHaveLength(read.events.length + 1)
  ...
  describe("the ref only moves forward", () => {
    // today: red — no change ref, so no event order to read. Measured: after
    ...
    it("a merged change reads opened, checked, merged, sent, and the merged event names the merge commit", async () => {
    ...
    it("a failed change ends with one failed event carrying the check, the fault and the remedy", async () => {
      ...
      const report = `${runSummary(run)}\n${read.report}`
      expect(read.exists, report).toBe(true)
      // Whether a `checked` event precedes the failure depends on which stage
      ...
      const failed = read.events.find((event) => event.kind === "failed")
    ...
    it("a stuck change ends with one stuck event and its reason, with no fault line: stuck is always the queue's", async () => {
      ...
      const stuck = read.events.find((event) => event.kind === "stuck")
    ...
    // keep off it.
    it("every event after the first has one parent, the event before it, and the merge commit is never a parent", async () => {
      ...
      const report = `${runSummary(run)}\n${read.report}`
      expect(read.exists, report).toBe(true)
      expect(read.events.length, report).toBeGreaterThan(1)

      for (const [index, event] of read.events.entries()) {
        if (index === 0) continue
        expect(event.parents, `${report}\nevent ${String(index)} (${event.kind})`).toEqual([read.events[index - 1]?.sha])
      }
      ...
      expect(
        read.events.flatMap((event) => event.parents),
        report,
      ).not.toContain(mergeCommit)
    })

    // today: red — no change ref, so nothing to preserve or to amend.
    it("a queue run only appends: every event it found is still there, at the same sha, and no ref was deleted", async () => {
      const { boundary, change } = await submitted({ exit: 0, hooks: true }, "append")

      const before = await readEvents(boundary, change)
      expect(before.exists, before.report).toBe(true)
      const refsBefore = await refs(boundary.origin)

      const run = await queueRunOnce(boundary.repo)

      const after = await readEvents(boundary, change)
      const report = `${runSummary(run)}\n--- before ---\n${before.report}\n--- after ---\n${after.report}`
      expect(after.exists, report).toBe(true)
      // Written once, never amended: the earlier events are the same commits.
      expect(
        after.events.slice(0, before.events.length).map((event) => event.sha),
        report,
      ).toEqual(before.events.map((event) => event.sha))
      // Forward only.
    ...
    // today: red — no change ref, so nothing bounds what a Event: may say.
    it("every event commit carries exactly one Event:, and its value is one of the six", async () => {
      ...
      const report = `${runSummary(run)}\n${read.report}`
      expect(read.exists, report).toBe(true)
      expect(read.events.length, report).toBeGreaterThan(0)

      for (const event of read.events) {
        expect(event.trailers.get("Event"), `${report}\nevent ${event.sha}`).toHaveLength(1)
        expect(EVENT_KINDS.has(event.kind), `${report}\nevent ${event.sha} says Event: ${event.kind}`).toBe(true)
      }
      // The genesis is not an event and carries none.
      expect(read.genesis?.trailers.get("Event"), report).toBeUndefined()
    })
  })
  ...
  describe("a retry, and a new head", () => {
    ...
    it("a submit at an unchanged head appends a second opened event and moves nothing else", async () => {
      const { boundary, change } = await submitted({ exit: 0 }, "retry")

      const first = await readEvents(boundary, change)
      ...
      const second = await readEvents(boundary, change)
      const report = `${again.report}\n--- before ---\n${first.report}\n--- after ---\n${second.report}`
      expect(second.exists, report).toBe(true)
      expect(second.kinds, report).toEqual([...first.kinds, "opened"])
      // Nothing moved: the earlier events are the same commits.
      expect(
        second.events.slice(0, first.events.length).map((event) => event.sha),
        report,
      ).toEqual(first.events.map((event) => event.sha))
      // A retry's opened event is a later event, so it has one parent.
      expect(second.events.at(-1)?.parents, report).toEqual([first.tip])
    ...
    it("a new head is a new change, and the change at the old head is left standing", async () => {
      ...
      const opened = await readEvents(boundary, { branch: change.branch, headSha: newHead })
      const still = await readEvents(boundary, change)
      const report = `${again.report}\n--- new head ---\n${opened.report}\n--- old head ---\n${still.report}`

      expect(opened.exists, report).toBe(true)
      expect(opened.kinds, report).toEqual(["opened"])
      expect(opened.events[0]?.trailers.get("Change")?.[0] ?? "", report).toBe(`${change.branch}@${newHead}`)

      // yrd deletes nothing: the change at the old head is still there, whole.
      expect(still.exists, report).toBe(true)
      expect(
        still.events.map((event) => event.sha),
        report,
      ).toEqual(first.events.map((event) => event.sha))
  ...
  describe("the tip event is the whole answer", () => {
    ...
    it("one for-each-ref over the change refs answers state, change and target with no history walk", async () => {
      ...
      for (const key of ["Event", "Change", "Target"]) {
      ...
      const state = await tipTrailer(boundary.origin, "Event")
      expect(EVENT_KINDS.has(state[0] ?? ""), `${runSummary(run)}\nEvent: ${state[0] ?? "(none)"}`).toBe(true)
    })
  })
})
